...
func _turn():
	if Input.is_action_just_pressed("mouse1"):
		if $".."._get_range(tilemapPosition, movementRange).has($"..".world_to_map(get_viewport().get_mouse_position())):
			print("GOTCHEE" + str($"..".world_to_map(get_viewport().get_mouse_position())))
		else:
			print("F" + str($"..".world_to_map(get_viewport().get_mouse_position())))
		_finish_turn()
	pass
